...
async function mirrorNewest(
  ...
  const candidates = events
    .filter(
      (ev) => ev.pubkey === pubkey && (ev.kind === 0 || ev.kind === 30023),
    )
...
async function syncFromRelays(env: Env, pubkey: string): Promise<void> {
  const events = await fetchEvents(
    relayList(env),
    { kinds: [0, 30023], authors: [pubkey], limit: NPUB_MIRROR_CAP },
 ...
 * behind the per-pubkey cooldown marker.
 */
...
mainRoutes.get(`${NPUB_PARAM}/:slug`, async (c) => {
  ...
  return c.html(
    PostPage({
      ...
      bodyHtml: row.rendered ?? renderPost(row.content),
      themeCss: "",
      mainHost,
      basePath: `/${r.npub}`,
    }),
