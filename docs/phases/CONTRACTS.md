...
- **Challenge issuance budget**: per-IP challenge limit lowered 30 → 10/15min (login itself only permits 10/15min/IP), and a NEW global cap of 500 challenges/day (D1 `rate_limits` key `challenge:global`, same pattern as the P3 npub-mirror budget). Every challenge is a KV put and every consumed nonce a KV delete against the 1,000-writes/day free-tier budget shared with sessions and gen bumps; without the global cap a handful of IPs could legally exhaust it and break login + cache invalidation platform-wide.
- **Nonce single-use is best-effort, not strict** (documented residual, accepted for now): KV get→delete is non-atomic and eventually consistent (~60s cross-colo), so a captured signed login event can in principle be replayed within that window. Exploitation requires possession of the victim's signed event and yields sessions only for that pubkey. Strict single-use requires moving nonces from KV to D1 (atomic `DELETE … RETURNING`) — a contract change (P4 brief mandates "nonce in KV") left to the orchestrator; it would also further shrink the KV write-budget exposure above.
